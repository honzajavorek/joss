 ...
 * Nette Framework
 *
 * Copyright (c) 2004, 2008 David Grudl aka -dgx- (http://www.davidgrudl.com)
 *
 ...
 * @copyright  Copyright (c) 2004, 2008 David Grudl
 * @package    Nette::Web
 * @version    $Revision: 101 $ $Date: 2008-01-08 07:43:13 +0100 (čt, 08 I 2008) $
...
class NHtml extends NObject implements ArrayAccess, Countable, IteratorAggregate
{
    ...
    final public function setText($text, $isHtml = FALSE)
    {
        if ($text === NULL) {
            ...
            throw new Exception("Textual content must be a scalar");
        }
    ...
    final public function insert($index, $child, $replace = FALSE)
    {
        if ($child instanceof NHtml) {
            if ($child->parent !== NULL) {
                throw new Exception('Child node already has parent');
            }
            $child->parent = $this;

        } elseif (!is_string($child)) {
            throw new Exception('Child node must be scalar or NHtml object');
        }
