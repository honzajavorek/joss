...
// NOTE some parts deleted (classes such as Object, Html, etc.), some names changed
...
function
__construct(IContainer$parent=NULL,$id=NULL){if($parent!==NULL){$parent->addComponent($this,$id);}elseif(is_string($id)){$this->uniqueId=$this->id=$id;}if($this->serviceLocator===NULL){$this->serviceLocator=ServiceLocator::getInstance();}$this->constructed();}protected
function
constructed(){}public
function
...
instanceof
Container&&$this->parent->isCloning())){$this->setParent(NULL);}}final
public
function
__sleep(){throw
new
...
ServiceLocator($this->serviceLocator));}catch(Exception$e){unset($this->components[$id]);throw$e;}}public
function
...
function
beforeHierarchyChange(IComponent$sender){parent::beforeHierarchyChange($sender);$this->notifyComponents('beforeHierarchyChange',array($sender),'Component',FALSE);}protected
function
afterHierarchyChange(IComponent$sender){parent::afterHierarchyChange($sender);$this->notifyComponents('afterHierarchyChange',array($sender),'Component',TRUE);}protected
function
refreshCache(){parent::refreshCache();$this->notifyComponents('refreshCache',array(),'Component');}protected
function
notifyComponents($method,array$args=array(),$type='Component',$depthFirst=TRUE){foreach($this->components
